...
- $\mu$ is a constant (the mean), 
- $\psi_j$ are real coefficients, 
- the infinite series converges in mean square.
...
where: 
- $\{Z_t\}$ is a white noise sequence with zero mean and finite variance,\
...
## Correlogram of MA(q) Process

-   The ACF of an MA(q) process has a **distinctive cut-off property**:
    -   Nonzero autocorrelations only at lags $0,1,\dots,q$.
    -   Zero autocorrelations for all lags $>q$.
-   **Identification rule**:
    -   Count the number of significant spikes in the ACF.
    -   The lag $q$ at which the ACF cuts off indicates the MA order.

### Example

-   If ACF is nonzero at lags 1 and 2, and zero afterwards → MA(2).
-   If ACF is nonzero only at lag 1 → MA(1).

## Example: MA(2) and its plots
...
## Operators Useful in Theoretical Development of Time Series Models
...
#### MA(q) in Backshift Operator Form
...
## Theory of Invertibility of MA Processes
...
$$
X_t = Z_t + \beta_1 Z_{t-1} + \beta_2 Z_{t-2} + \cdots + \beta_q Z_{t-q}
     = \beta(B) Z_t, \tag{*}
$$
...
- The representation in (**) is useful because the unobservable $Z_t$ is now expressed as a linear combination of the observable $X_{t-j}$’s.  
- This representation is known as the **invertible solution** to the MA($q$) model in (*).  
- In practice, invertibility ensures that we can estimate and work with $Z_t$ indirectly via the observed data $X_t$.
...
$$
Z_t = (1 - \beta B)^{-1} X_t 
    = \sum_{j=0}^\infty \beta^j B^j X_t 
    = \sum_{j=0}^\infty \beta^j X_{t-j}.
$$

## Invertible Solutions to MA($q$) Processes
...
$$
[\beta(B)]^{-1} = I + a_1 B + a_2 B^2 + \cdots,
$$
so that

$$
Z_t = \Big(\sum_{j=0}^\infty a_j B^j\Big) X_t
...
$$
Z_t = \beta(B)^{-1} X_t = \sum_{j=0}^\infty a_j X_{t-j}, 
\quad \sum_{j=0}^\infty |a_j|^2 < \infty.
$$